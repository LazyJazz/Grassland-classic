...
void VertexArray::BindVerticesData(void *vertices_data,
                                   int32_t data_count,
                                   GRL_OPENGL_BUFFER_USAGE usage) {
  // GRLGLCall(glBindVertexArray(__vertex_array_object));
  GRLGLCall(glBindBuffer(GL_ARRAY_BUFFER, __vertex_buffer_object));
  GRLGLCall(glBufferData(GL_ARRAY_BUFFER, sizeof(float) * data_count,
                         vertices_data, __OpenGLLifecycleEnumToMacro(usage)));
  // GRLGLCall(glBindVertexArray(0));
}
void VertexArray::BindIndicesData(void *indices_data,
                                  int32_t data_count,
                                  GRL_OPENGL_BUFFER_USAGE usage) {
  GRLGLCall(glBindVertexArray(__vertex_array_object));
  GRLGLCall(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * data_count,
                         indices_data, __OpenGLLifecycleEnumToMacro(usage)));
  GRLGLCall(glBindVertexArray(0));
...
void VertexArray::ActiveVerticesLayout(int32_t slot,
                                       int32_t bundle_size,
                                       GRL_TYPE bundle_type,
                                       int32_t stride,
                                       int32_t offset) {
  GRLGLCall(glBindBuffer(GL_ARRAY_BUFFER, __vertex_buffer_object));
  GRLGLCall(glBindVertexArray(__vertex_array_object));
  GRLGLCall(glVertexAttribPointer(slot, bundle_size,
                                  __OpenGLTypeTranslate(bundle_type), GL_FALSE,
                                  stride, (void *)(offset)));
...
class GRLCOpenGLVertexArray : public GRLIOpenGLVertexArray {
 public:
  ...
  virtual void BindVerticesData(void *vertices_data,
                                int32_t data_count,
                                GRL_OPENGL_BUFFER_USAGE usage);
  virtual void BindIndicesData(void *indices_data,
                               int32_t data_count,
                               GRL_OPENGL_BUFFER_USAGE usage);
  virtual void ActiveVerticesLayout(int32_t slot,
                                    int32_t bundle_size,
                                    GRL_TYPE bundle_type,
                                    int32_t stride,
                                    int32_t offset);
...
GRL_RESULT GRLCreateOpenGLVertexArray(GRLIOpenGLVertexArray **ppVertexArray) {
  if (ppVertexArray)
    *ppVertexArray =
        new GRLCOpenGLVertexArray(new Graphics::OpenGL::VertexArray());
  else
    return GRL_TRUE;
  return GRL_FALSE;
}
GRLCOpenGLVertexArray::GRLCOpenGLVertexArray(
    Graphics::OpenGL::VertexArray *pVertexArray) {
  __Ref_Cnt = 1;
...
void GRLCOpenGLVertexArray::BindVerticesData(void *vertices_data,
                                             int32_t data_count,
                                             GRL_OPENGL_BUFFER_USAGE usage) {
  __vertex_array->BindVerticesData(vertices_data, data_count, usage);
}
void GRLCOpenGLVertexArray::BindIndicesData(void *indices_data,
                                            int32_t data_count,
                                            GRL_OPENGL_BUFFER_USAGE usage) {
  __vertex_array->BindIndicesData(indices_data, data_count, usage);
}
void GRLCOpenGLVertexArray::ActiveVerticesLayout(int32_t slot,
                                                 int32_t bundle_size,
                                                 GRL_TYPE bundle_type,
                                                 int32_t stride,
                                                 int32_t offset) {
  __vertex_array->ActiveVerticesLayout(slot, bundle_size, bundle_type, stride,
                                       offset);
}
