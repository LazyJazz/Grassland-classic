...
GRL_RESULT Program::LinkShaders(Shader *vertex_shader,
  ...
  if (vertex_shader)
    vertex_shader_handle = vertex_shader->GetShaderHandle();
  if (fragment_shader)
    fragment_shader_handle = fragment_shader->GetShaderHandle();
  return LinkShaders(vertex_shader_handle, fragment_shader_handle,
                     geometry_shader_handle);
}

GRL_RESULT Program::LinkShaders(uint32_t __vertex_shader_handle,
...
GRL_RESULT Shader::CompileShaderFromFile(const char *shader_file_path,
                                         GRL_OPENGL_SHADER_TYPE shader_type) {
  ...
  __shader_handle = glCreateShader(__OpenGLShaderTypeEnumToValue(shader_type));
...
class GRLCOpenGLProgram : public GRLIOpenGLProgram {
 public:
  ...
  virtual GRL_RESULT SetVec2(const char *uniform_name,
                             const GRLVec2 &vec) const;
  virtual GRL_RESULT SetVec3(const char *uniform_name,
                             const GRLVec3 &vec) const;
  virtual GRL_RESULT SetVec4(const char *uniform_name,
                             const GRLVec4 &vec) const;
  virtual GRL_RESULT SetMat2(const char *uniform_name,
                             const GRLMat2 &mat) const;
  virtual GRL_RESULT SetMat3(const char *uniform_name,
                             const GRLMat3 &mat) const;
  virtual GRL_RESULT SetMat4(const char *uniform_name,
                             const GRLMat4 &mat) const;
...
GRL_RESULT GRLCompileOpenGLShaderFromFile(const char *shader_file_path,
                                          GRL_OPENGL_SHADER_TYPE shader_type,
...
GRL_RESULT GRLCreateOpenGLLinkedProgram(GRLIOpenGLShader *pVertexShader,
  ...
  if (pProgram->LinkShaders(vertex_shader, fragment_shader, geometry_shader)) {
    delete pProgram;
    *ppProgram = NULL;
    return GRL_TRUE;
  }
...
GRL_RESULT GRLCreateOpenGLProgramFromSourceFile(
  ...
  if (vertex_shader_path) {
    if (GRLCompileOpenGLShaderFromFile(vertex_shader_path,
  ...
  if (fragment_shader_path) {
    if (GRLCompileOpenGLShaderFromFile(fragment_shader_path,
  ...
  if (geometry_shader_path) {
    if (GRLCompileOpenGLShaderFromFile(geometry_shader_path,
  ...
  if (GRLCreateOpenGLLinkedProgram(pVertexShader, pFragmentShader,
                                   pGeometryShader, ppProgram)) {
    if (pVertexShader)
      pVertexShader->Release();
    if (pFragmentShader)
      pFragmentShader->Release();
...
GRL_RESULT GRLCOpenGLShader::GetShaderHandle(
    uint32_t *__shader_handle_pnter) const {
  if (!__shader_pnter) {
    GRLSetErrorInfo("OpenGL Shader Object not initialized. ");
    return GRL_TRUE;
  }
...
GRL_RESULT GRLCOpenGLProgram::SetVec2(int32_t location,
                                      const GRLVec2 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetVec3(int32_t location,
                                      const GRLVec3 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetVec4(int32_t location,
                                      const GRLVec4 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat2(int32_t location,
                                      const GRLMat2 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat3(int32_t location,
                                      const GRLMat3 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat4(int32_t location,
                                      const GRLMat4 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetFloat(const char *uniform_name,
                                       float fvalue) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetInt(const char *uniform_name,
                                     int value) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetVec2(const char *uniform_name,
                                      const GRLVec2 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetVec3(const char *uniform_name,
                                      const GRLVec3 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetVec4(const char *uniform_name,
                                      const GRLVec4 &vec) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat2(const char *uniform_name,
                                      const GRLMat2 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat3(const char *uniform_name,
                                      const GRLMat3 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
...
GRL_RESULT GRLCOpenGLProgram::SetMat4(const char *uniform_name,
                                      const GRLMat4 &mat) const {
  if (!__program_pnter) {
    GRLSetErrorInfo("OpenGL Program Object not initialized. ");
    return GRL_TRUE;
