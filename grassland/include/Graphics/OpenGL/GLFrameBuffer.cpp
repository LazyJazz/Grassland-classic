...
uint32_t __OpenGLSlotTranslate(GRL_OPENGL_FRAMEBUFFER_SLOT slot) {
  ...
  if (slot >= GRL_OPENGL_FRAMEBUFFER_SLOT_COLOR0 &&
      slot <= GRL_OPENGL_FRAMEBUFFER_SLOT_COLOR_LAST)
...
class GRLCOpenGLFrameBuffer : public GRLIOpenGLFrameBuffer {
 public:
  ...
  virtual GRL_RESULT BindTexture(
      GRLIOpenGLTexture *pTexture,
      GRL_OPENGL_FRAMEBUFFER_SLOT slot = GRL_OPENGL_FRAMEBUFFER_SLOT_COLOR0);
...
GRL_RESULT GRLCOpenGLFrameBuffer::CheckStatus() const {
  if (glCheckNamedFramebufferStatus(__framebuffer_handle, GL_FRAMEBUFFER) ==
      GL_FRAMEBUFFER_COMPLETE)
    return GRL_FALSE;
  return GRL_TRUE;
}

GRL_RESULT GRLCOpenGLFrameBuffer::BindTexture(
    GRLIOpenGLTexture *_pTexture,
  ...
  } else if (slot >= GRL_OPENGL_FRAMEBUFFER_SLOT_COLOR0 &&
             slot <= GRL_OPENGL_FRAMEBUFFER_SLOT_COLOR_LAST) {
...
GRLPtr<GRLIOpenGLFrameBuffer> g_active_framebuffer(
    (GRLIOpenGLFrameBuffer *)nullptr);
...
GRL_RESULT GRLOpenGLGetFrameBufferSize(int32_t *width, int32_t *height) {
  if (g_active_framebuffer.Get()) {
    if (width)
      *width = g_active_framebuffer->GetWidth();
    if (height)
      *height = g_active_framebuffer->GetHeight();
  } else {
    GRLOpenGLGetWindowSize(width, height);
  }
  return GRL_FALSE;
