...
uint32_t GRLExtensionModeToOpenGLWrapMode(
  ...
  switch (extension) {
    case GRL_GRAPHICS_TEXTURE_EXTENSION_MODE::BLACK:
...
GRL_RESULT GRLCOpenGLEnvironment::CreateDepthMap(
    uint32_t width,
    uint32_t height,
...
GRL_RESULT GRLCOpenGLEnvironment::CreateBuffer(uint64_t size,
...
GRL_RESULT GRLCOpenGLEnvironment::CreatePipelineState(
...
GRL_RESULT GRLCOpenGLEnvironment::ApplyPipelineState(
  ...
  if (m_workingPipelineState->m_enableDepthTest)
    GRLOpenGLCall(glEnable, GL_DEPTH_TEST);
  else
    GRLOpenGLCall(glDisable, GL_DEPTH_TEST);
  if (m_workingPipelineState->m_enableBlend)
    GRLOpenGLCall(glEnable, GL_BLEND);
  else
    GRLOpenGLCall(glDisable, GL_BLEND);
  if (m_workingPipelineState->m_cullFace)
    GRLOpenGLCall(glEnable, GL_CULL_FACE);
  else
    GRLOpenGLCall(glDisable, GL_CULL_FACE);
  if (m_workingPipelineState->m_cullFace == 1)
    GRLOpenGLCall(glCullFace, GL_BACK);
  else
    GRLOpenGLCall(glCullFace, GL_FRONT);
  return GRL_FALSE;
}

GRL_RESULT GRLCOpenGLEnvironment::SetViewport(uint32_t x,
                                              uint32_t y,
                                              uint32_t width,
                                              uint32_t height) {
  if (!m_duringDraw)
...
GRL_RESULT GRLCOpenGLEnvironment::SetConstantBuffer(
  ...
  if (constantBuffer->QueryInterface(GRLID_PPV_ARGS(&pConstantBuffer)))
    return GRL_TRUE;
  GRLOpenGLCall(glBindBufferBase, GL_UNIFORM_BUFFER, constantBufferIndex,
                pConstantBuffer->m_buffer);
  return GRL_FALSE;
}

GRL_RESULT GRLCOpenGLEnvironment::SetTextures(
    uint32_t numTexture,
    GRLIGraphicsTexture *const *pTextures) {
  if (!m_duringDraw)
    return GRL_TRUE;
  MakeThisContextCurrent();
  GRLPtr<GRLCOpenGLTexture> *textures =
      new GRLPtr<GRLCOpenGLTexture>[numTexture];
  ...
  for (int i = 0; i < numTexture; i++) {
    ...
    GRLOpenGLCall(glTexParameteri, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  GRLFilterToOpenGLFilter(
                      m_workingPipelineState->m_samplerDesc[i].filter));
    GRLOpenGLCall(glTexParameteri, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                  GRLFilterToOpenGLFilter(
                      m_workingPipelineState->m_samplerDesc[i].filter));
    GRLOpenGLCall(glTexParameteri, GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  GRLExtensionModeToOpenGLWrapMode(
                      m_workingPipelineState->m_samplerDesc[i].extensionU));
    GRLOpenGLCall(glTexParameteri, GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                  GRLExtensionModeToOpenGLWrapMode(
                      m_workingPipelineState->m_samplerDesc[i].extensionV));
    GRLOpenGLCall(glTexParameteri, GL_TEXTURE_2D, GL_TEXTURE_WRAP_R,
                  GRLExtensionModeToOpenGLWrapMode(
                      m_workingPipelineState->m_samplerDesc[i].extensionW));
    GRLOpenGLCall(glBindTexture, GL_TEXTURE_2D, 0);
  }
  for (int i = 0; i < numTexture; i++) {
    GRLOpenGLCall(glActiveTexture, GL_TEXTURE0 + i);
    GRLOpenGLCall(glBindTexture, GL_TEXTURE_2D, textures[i]->m_texture);
    GRLOpenGLCall(glUniform1i, m_workingPipelineState->GetTextureLocation(i),
                  i);
  }
...
GRL_RESULT GRLCOpenGLEnvironment::SetRenderTargets(
  ...
  GRLOpenGLCall(glBindFramebuffer, GL_FRAMEBUFFER,
                m_workingPipelineState->m_frameBuffer);
  if (pDepthMap) {
    GRLOpenGLCall(glFramebufferTexture2D, GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                  GL_TEXTURE_2D, depthMap->m_depthMap, 0);
  }
  uint32_t colorAttachments[32] = {};
  for (int i = 0; i < numRenderTarget; i++) {
    GRLOpenGLCall(glFramebufferTexture2D, GL_FRAMEBUFFER,
                  GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D,
                  renderTargets[i]->m_texture, 0);
...
GRL_RESULT GRLCOpenGLEnvironment::DrawInstance(
  ...
  size_t offset = 0;
  for (int index = 0; index < m_workingPipelineState->m_numInputElement;
       index++) {
    // std::cout << "input index: " << index << std::endl;
    GRLOpenGLCall(
        glVertexAttribPointer, index,
        GRLFormatElementNumber(m_workingPipelineState->m_inputLayout[index]),
        GRLFormatToOpenGLElementType(
            m_workingPipelineState->m_inputLayout[index]),
        GRL_FALSE, m_workingPipelineState->m_strideSizeInBytes, (void *)offset);
    offset +=
        GRLFormatSizeInBytes(m_workingPipelineState->m_inputLayout[index]);
    GRLOpenGLCall(glEnableVertexAttribArray, index);
  }
  for (int index = m_workingPipelineState->m_numInputElement;
       index < m_lastActiveAttrib; index++) {
    GRLOpenGLCall(glDisableVertexAttribArray, index);
  }
  m_lastActiveAttrib = m_workingPipelineState->m_numInputElement;
  GRLOpenGLCall(glDrawArrays, GRLRenderTopologyToOpenGLTopology(renderTopology),
                0, numVertices);
...
GRL_RESULT GRLCOpenGLEnvironment::DrawIndexedInstance(
    GRLIGraphicsBuffer *pVertexBuffer,
  ...
  m_lastActiveAttrib = m_workingPipelineState->m_numInputElement;
  GRLOpenGLCall(glDrawElements,
                GRLRenderTopologyToOpenGLTopology(renderTopology), numVertices,
...
GRL_RESULT GRLCOpenGLEnvironment::QueryInterface(GRLUUID Uuid,
                                                 void **ppObject) {
  if (Uuid == GRLID_IBase)
    *ppObject = reinterpret_cast<GRLIBase *>(this);
  else if (Uuid == GRLID_IGraphicsEnvironment)
    *ppObject = reinterpret_cast<GRLIGraphicsEnvironment *>(this);
  else if (Uuid == GRLID_COpenGLEnvironment)
    *ppObject = reinterpret_cast<GRLCOpenGLEnvironment *>(this);
  else
    GRL_TRUE;
  AddRef();
  return GRL_FALSE;
...
GRLCOpenGLEnvironment::GRLCOpenGLEnvironment(uint32_t width,
                                             uint32_t height,
                                             const char *window_title) {
  __Ref_Cnt = 1;
  m_swapInterval = -1;
...
#endif
  m_window =
      glfwCreateWindow(width, height, strtitle.c_str(), nullptr, nullptr);
  if (!m_window) {
...
GRL_RESULT GRLCOpenGLEnvironment::CreateTexture(
    uint32_t width,
    uint32_t height,
    GRL_FORMAT format,
...
GRLCOpenGLDepthMap::GRLCOpenGLDepthMap(uint32_t width,
                                       uint32_t height,
  ...
  GRLOpenGLCall(glTexImage2D, GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, width,
...
GRL_RESULT GRLCOpenGLDepthMap::QueryInterface(GRLUUID Uuid, void **ppObject) {
  ...
  else if (Uuid == GRLID_COpenGLDepthMap)
    *ppObject = reinterpret_cast<GRLCOpenGLDepthMap *>(this);
  else
    return GRL_TRUE;
  AddRef();
  return GRL_FALSE;
...
Grassland::GRLCOpenGLTexture::GRLCOpenGLTexture(
  ...
  GRLOpenGLCall(glTexImage2D, GL_TEXTURE_2D, 0, m_internalFormat, width, height,
                0, GRLFormatToOpenGLBaseFormat(format),
                GRLFormatToOpenGLElementType(format), nullptr);
  GRLOpenGLCall(glBindTexture, GL_TEXTURE_2D, 0);
  m_environment.Set(pEnvironment);
}
...
GRL_RESULT Grassland::GRLCOpenGLTexture::WritePixels(void *pData) {
  ...
  for (int i = 0; i < m_height; i++)
    memcpy(pReversedData + i * (m_width * pixelSize),
           pRealData + (m_height - 1 - i) * (m_width * pixelSize),
           pixelSize * m_width);
  GRLOpenGLCall(glTextureSubImage2D, m_texture, 0, 0, 0, m_width, m_height,
                GRLFormatToOpenGLBaseFormat(m_format),
                GRLFormatToOpenGLElementType(m_format), pReversedData);
...
GRL_RESULT GRLCOpenGLTexture::ReadPixels(void *pData) {
  ...
  uint8_t *pRealData = reinterpret_cast<uint8_t *>(pData);
  GRLOpenGLCall(
      glGetTextureImage, m_texture, 0, GRLFormatToOpenGLBaseFormat(m_format),
      GRLFormatToOpenGLElementType(m_format),
      m_width * m_height * GRLFormatSizeInBytes(m_format), pReversedData);
  for (int i = 0; i < m_height; i++)
    memcpy(pRealData + i * (m_width * pixelSize),
           pReversedData + (m_height - 1 - i) * (m_width * pixelSize),
           pixelSize * m_width);
  delete[] pReversedData;
  return GRL_FALSE;
}
...
GRL_RESULT GRLCOpenGLTexture::QueryInterface(GRLUUID Uuid, void **ppObject) {
  if (Uuid == GRLID_IBase)
    ...
    *ppObject = reinterpret_cast<GRLIGraphicsTexture *>(this);
  else if (Uuid == GRLID_COpenGLTexture)
    *ppObject = reinterpret_cast<GRLCOpenGLTexture *>(this);
  else
    return GRL_TRUE;
  AddRef();
  return GRL_FALSE;
...
GRLCOpenGLPipelineState::GRLCOpenGLPipelineState(
  ...
  uint32_t fragmentShader = GRLOpenGLCompileShaderFromFile(
               fragment_shader_path.c_str(), GL_FRAGMENT_SHADER),
           vertexShader = GRLOpenGLCompileShaderFromFile(
               vertex_shader_path.c_str(), GL_VERTEX_SHADER);
  ...
  if (success != GL_TRUE) {
    int32_t log_length = 0;
    char message[1024];
    GRLOpenGLCall(glGetProgramInfoLog, m_shaderProgram, 1000, &log_length,
                  message);
  ...
  for (int index = 0; index < desc->numInputElement; index++) {
    m_strideSizeInBytes +=
        GRLFormatSizeInBytes(desc->inputElementLayout[index]);
...
GRL_RESULT GRLCOpenGLPipelineState::QueryInterface(GRLUUID Uuid,
                                                   void **ppObject) {
  if (Uuid == GRLID_IBase)
    *ppObject = reinterpret_cast<GRLIBase *>(this);
  else if (Uuid == GRLID_IGraphicsPipelineState)
    *ppObject = reinterpret_cast<GRLIGraphicsPipelineState *>(this);
  else if (Uuid == GRLID_COpenGLPipelineState)
    *ppObject = reinterpret_cast<GRLCOpenGLPipelineState *>(this);
  else
    return GRL_TRUE;
  AddRef();
  return GRL_FALSE;
...
int GRLCOpenGLPipelineState::GetTextureLocation(int textureIndex) {
  if (!m_mapTextureLocation.count(textureIndex)) {
    ...
    m_mapTextureLocation[textureIndex] =
        glGetUniformLocation(m_shaderProgram, textureName.c_str());
  }
  return m_mapTextureLocation[textureIndex];
}
GRLCOpenGLBuffer::GRLCOpenGLBuffer(uint64_t size,
                                   GRL_GRAPHICS_BUFFER_TYPE type,
                                   GRL_GRAPHICS_BUFFER_USAGE usage,
                                   void *pData,
                                   GRLCOpenGLEnvironment *pEnvironment) {
...
GRL_RESULT GRLCOpenGLBuffer::WriteData(uint64_t size,
                                       uint64_t offset,
                                       void *pData) {
  m_environment->MakeThisContextCurrent();
...
GRL_RESULT GRLCOpenGLBuffer::QueryInterface(GRLUUID Uuid, void **ppObject) {
  ...
  else if (Uuid == GRLID_COpenGLBuffer)
    *ppObject = reinterpret_cast<GRLCOpenGLBuffer *>(this);
  else
    return GRL_TRUE;
  AddRef();
  return GRL_FALSE;
