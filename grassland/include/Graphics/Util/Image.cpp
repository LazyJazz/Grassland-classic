...
void Image::Resize(int32_t width, int32_t height) {
  if (this->width == width && this->height == height)
    return;
  int32_t copy_width = std::min(this->width, width),
          copy_height = std::min(this->height, height);
  Color *pColorBuffer_new = new Color[width * height];
  for (int y = 0; y < copy_height; y++)
    memcpy(pColorBuffer_new + (y * width), pColorBuffer + (y * this->width),
...
uint32_t Image::LoadBMP(const char *_path) {
  ...
  if (fileheader.bfReserved2 != 0)
    wrong_format = true;
  if (fileheader.bfOffBits != 0x36)
    wrong_format = true;
  if (infoheader.biSize != 0x28)
    wrong_format = true;
  if (infoheader.biPlanes != 1)
    wrong_format = true;
  if (infoheader.biBitCount != 0x18)
    wrong_format = true;
  ...
  for (int y = rh - 1; y >= 0; y--) {
    ...
    for (int x = 0; x < width; x++)
      pColorBuffer[y * width + x] =
          Color(__ReadBuf[x * 3 + 2] * (1.0f / 255.0f),
