...
class CameraNormal : public Camera, public Coord<__Ty> {
 public:
  ...
  /*
   * @param afov: Field of view angle, how wide you can see vertically. Should
   * be in (0, 2 * pi).
   *
   * @param aspect: Should be set as screen_width / screen_height to get correct
   * photo.
   *
...
template <typename __Ty>
inline void Coord<__Ty>::LookAt(Math::Vector<3, __Ty> from,
...
template <typename __Ty>
inline CameraNormal<__Ty>::CameraNormal(__Ty afov,
                                        __Ty aspect,
                                        __Ty Near,
                                        __Ty Far) {
...
inline GRLRay CameraNormal<__Ty>::GenRay(float x, float y, float z) const {
  ...
  GRLVec3 d(x, y, z);
  d = d / d.norm();
  return GRLRay(GRLVec3(Coord<__Ty>::__coord[0][3], Coord<__Ty>::__coord[1][3],
...
inline GRLRayd CameraNormal<__Ty>::GenRay(double x, double y, double z) const {
  ...
  d = d / d.norm();
  return GRLRayd(
      GRLVec3d(Coord<__Ty>::__coord[0][3], Coord<__Ty>::__coord[1][3],
               Coord<__Ty>::__coord[2][3]),
      d);
}
...
template <typename __Ty>
inline void CameraNormal<__Ty>::SetFOV(__Ty afov,
                                       __Ty aspect,
                                       __Ty Near,
                                       __Ty Far) {
  __afov = tan(afov * 0.5);
