...
enum class GRL_RENDER_TOPOLOGY { TRIANGLE = 0, LINE = 1 };

enum class GRL_GRAPHICS_API { D3D12 = 0, OPENGL = 1, VULKAN = 2 };
...
enum class GRL_GRAPHICS_BUFFER_USAGE { DEFAULT = 0, DYNAMIC = 1, STATIC = 2 };
...
GRLMakeObjectUUIDAssociate(GRLIGraphicsEnvironment,
                           GRLID_IGraphicsEnvironment,
                           "8c7edbfd-fc7b-479d-87ac-069480d65d3e");
GRLMakeObjectUUIDAssociate(GRLIGraphicsBuffer,
                           GRLID_IGraphicsBuffer,
                           "cb318405-fa08-4ded-8393-67dbd259e63f");
GRLMakeObjectUUIDAssociate(GRLIGraphicsTexture,
                           GRLID_IGraphicsTexture,
                           "29433e84-b78d-4a5d-886b-864ccef99acc");
GRLMakeObjectUUIDAssociate(GRLIGraphicsDepthMap,
                           GRLID_IGraphicsDepthMap,
                           "82d47f8c-f737-4584-8e00-7abae3df9804");
GRLMakeObjectUUIDAssociate(GRLIGraphicsPipelineState,
                           GRLID_IGraphicsPipelineState,
                           "18adfb02-d6b5-45e6-a7e6-018cfe1c1a62");
...
class GRLIGraphicsEnvironment : public GRLIBase {
 public:
  ...
  virtual GRL_RESULT ApplyPipelineState(
      GRLIGraphicsPipelineState *pPipelineState) = 0;
  virtual GRL_RESULT SetViewport(uint32_t x,
                                 uint32_t y,
                                 uint32_t width,
                                 uint32_t height) = 0;
  virtual GRL_RESULT SetConstantBuffer(uint32_t constantBufferIndex,
                                       GRLIGraphicsBuffer *constantBuffer) = 0;
  virtual GRL_RESULT SetTextures(uint32_t numTexture,
                                 GRLIGraphicsTexture *const *pTextures) = 0;
  virtual GRL_RESULT SetRenderTargets(
      uint32_t numRenderTarget,
      GRLIGraphicsTexture *const *pRenderTargets,
      GRLIGraphicsDepthMap *pDepthMap) = 0;
...
GRL_RESULT GRLCreateGraphicsEnvironment(
    uint32_t width,
    uint32_t height,
    const char *window_title,
    GRL_GRAPHICS_API graphicsAPI,
    GRLIGraphicsEnvironment **ppEnvironment);

GRLDeclareObject(IGraphics2DEnvironment,
                 "cacf4e12-69ce-41a3-9c73-906e9f50e666");
