...
GRLMakeObjectUUIDAssociate(GRLCD3D12Environment,
                           GRLID_CD3D12Environment,
                           "877751a2-5308-416e-9fee-c981d70964d5");
GRLMakeObjectUUIDAssociate(GRLCD3D12Buffer,
                           GRLID_CD3D12Buffer,
                           "fc0f8ae9-520d-44c5-9d85-54c4066dd6f7");
GRLMakeObjectUUIDAssociate(GRLCD3D12Texture,
                           GRLID_CD3D12Texture,
                           "7bb45f0c-5e72-459e-82a7-20c3160720d8");
GRLMakeObjectUUIDAssociate(GRLCD3D12DepthMap,
                           GRLID_CD3D12DepthMap,
                           "ea8ca3e4-1203-4aa3-b86f-7a60f3e5ad3f");
GRLMakeObjectUUIDAssociate(GRLCD3D12PipelineState,
                           GRLID_CD3D12PipelineState,
                           "70519a5b-9f40-491d-9bc4-ecd2676d966b");
...
class GRLCD3D12Environment : public GRLIGraphicsEnvironment {
 public:
  GRLCD3D12Environment(uint32_t width,
                       uint32_t height,
  ...
  GRL_RESULT AcquireSRVHandle(D3D12_CPU_DESCRIPTOR_HANDLE &cpuHandle,
                              D3D12_GPU_DESCRIPTOR_HANDLE &gpuHandle);
  GRL_RESULT ReturnSRVHandle(D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle,
                             D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle);
 ...
 private:
  static LRESULT WINAPI GRLD3D12EnvironmentProcFunc(HWND hWnd,
                                                    UINT Msg,
                                                    WPARAM wParam,
                                                    LPARAM lParam);
...
class GRLCD3D12Texture : public GRLIGraphicsTexture {
 public:
  GRLCD3D12Texture(uint32_t width,
                   uint32_t height,
                   GRL_FORMAT format,
                   GRLCD3D12Environment *pEnvironment);
  ~GRLCD3D12Texture();
  ...
  void ResourceStateTransition(ID3D12GraphicsCommandList *commandList,
...
class GRLCD3D12DepthMap : public GRLIGraphicsDepthMap {
 public:
  GRLCD3D12DepthMap(uint32_t width,
                    uint32_t height,
                    GRLCD3D12Environment *pEnvironment);
  virtual GRL_RESULT GetSize(uint32_t *pWidth, uint32_t *pHeight) override;
  virtual GRL_RESULT AddRef() override;
...
class GRLCD3D12PipelineState : public GRLIGraphicsPipelineState {
 public:
  GRLCD3D12PipelineState(GRLCD3D12Environment *pEnvironment,
                         const char *shader_path,
                         GRL_GRAPHICS_PIPELINE_STATE_DESC *desc);
  virtual GRL_RESULT AddRef() override;
  virtual GRL_RESULT Release() override;
