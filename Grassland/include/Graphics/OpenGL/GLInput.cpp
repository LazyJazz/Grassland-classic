...
namespace Grassland
{
	...
	void __GRLOpenGLCursorPosCallback(GLFWwindow* window, double xpos, double ypos)
	{
	...
	void __GRLOpenGLKeyCallback(GLFWwindow* window, int gl_key, int scannode, int action, int mods)
	{
		switch (action)
		{
		case GLFW_PRESS:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYDOWN, GRLOpenGLKey2GRLKey(gl_key), NULL);
			break;
		case GLFW_RELEASE:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYUP, GRLOpenGLKey2GRLKey(gl_key), NULL);
			break;
		case GLFW_REPEAT:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYLAST, GRLOpenGLKey2GRLKey(gl_key), NULL);
			break;
		}
	}

	void __GRLOpenGLMouseButtonCallback(GLFWwindow* window, int gl_mb, int action, int mods)
	{
		switch (action)
		{
		case GLFW_PRESS:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYDOWN, GRLOpenGLMouseButton2GRLKey(gl_mb), NULL);
			break;
		case GLFW_RELEASE:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYUP, GRLOpenGLMouseButton2GRLKey(gl_mb), NULL);
			break;
		case GLFW_REPEAT:
			g_GRLWinProc((uint64_t)window, GRL_WM_KEYLAST, GRLOpenGLMouseButton2GRLKey(gl_mb), NULL);
			break;
		}
	}

	void GRLOpenGLSetWindowProc(GRLWinProc WinProc, uint64_t param0, uint64_t param1)
	{
		g_GRLWinProc = WinProc;
		glfwSetCursorPosCallback(GRLOpenGLGetWindow(), __GRLOpenGLCursorPosCallback);
		glfwSetWindowSizeCallback(GRLOpenGLGetWindow(), __GRLOpenGLWindowSizeCallback);
		glfwSetKeyCallback(GRLOpenGLGetWindow(), __GRLOpenGLKeyCallback);
		glfwSetMouseButtonCallback(GRLOpenGLGetWindow(), __GRLOpenGLMouseButtonCallback);
