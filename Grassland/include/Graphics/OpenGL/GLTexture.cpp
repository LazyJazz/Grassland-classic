...
namespace Grassland
{
	namespace Graphics
	{
		namespace OpenGL
		{
			...
			Texture::Texture(int32_t width, int32_t height, GRL_OPENGL_TEXTURE_FORMAT format, void* data)
			{
				__texture_format = format;
				glGenTextures(1, &__texture_handle);
				glBindTexture(GL_TEXTURE_2D, __texture_handle);
				glTexImage2D(GL_TEXTURE_2D, 0, __OpenGLInternalFormatTranslate(format),
					width, height, 0, __OpenGLFormatTranslate(format), GL_FLOAT, data);
			...
			GRL_RESULT Texture::GetImage(Grassland::Graphics::Util::Image* img) const
			{
				...
				if (__texture_format == GRL_OPENGL_TEXTURE_FORMAT_DEPTH)
				{
					...
					GRLGLCall(glGetTextureImage(__texture_handle, 0, GL_DEPTH_COMPONENT, GL_FLOAT,
						sizeof(float) * _width * _height,
						depth_buffer));
				...
				{
					GRLGLCall(glGetTextureImage(__texture_handle, 0, GL_RGBA, GL_FLOAT, sizeof(
						Grassland::Graphics::Util::Color
						) * _width * _height,
						img->GetImageBuffer()));
				}
				return GRL_FALSE;
			...
			{
				glBindTexture(GL_TEXTURE_2D, __texture_handle);
				glTexImage2D(GL_TEXTURE_2D, 0, __OpenGLInternalFormatTranslate(__texture_format),
					width, height, 0, __OpenGLFormatTranslate(__texture_format), GL_FLOAT, nullptr);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	...
	{
		*ppTexture = new GRLCOpenGLTexture(new Graphics::OpenGL::Texture(width, height, format, init_data));
		return GRL_FALSE;
	}
	...
	GRL_RESULT GRLCOpenGLTexture::GetImage(GRLIImage* _pImg) const
	{
		...
		if (GetFormat() == GRL_OPENGL_TEXTURE_FORMAT_DEPTH)
		{
			...
			for (int y = 0; y < GetHeight(); y++)
				for (int x = 0; x < GetWidth(); x++)
					pBuffer[(GetHeight() - 1 - y) * GetWidth() + x] = Grassland::Graphics::Util::Color(
		...
		else
		{
			...
			GRLGLCall(glGetTextureImage(GetHandle(), 0, GL_RGBA, GL_FLOAT, sizeof(
				Grassland::Graphics::Util::Color
				) * size,
				depth_buffer));
			for (int y = 0; y < GetHeight(); y++)
				memcpy(
					pBuffer + (GetHeight() - 1 - y) * GetWidth(),
					depth_buffer + y * GetWidth(),
					GetWidth() * sizeof(GRLColor));
				/*for (int x = 0; x < GetWidth(); x++)
