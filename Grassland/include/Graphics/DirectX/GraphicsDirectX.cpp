...
namespace Grassland
{
	...
	uint32_t GRLCD3D12PipelineState::GetTextureRootParameterIndex(uint32_t textureIndex)
	{
		return textureIndex;
	}
	uint32_t GRLCD3D12PipelineState::GetConstantBufferRootParameterIndex(uint32_t constantBufferIndex)
	{
	...
	GRL_RESULT GRLCD3D12Environment::DrawInstance(GRLIGraphicsBuffer* pVertexBuffer, uint32_t numVertices, GRL_RENDER_TOPOLOGY renderTopology)
	{
		if (!m_duringDraw) return GRL_TRUE;
		GRLPtr<GRLCD3D12Buffer> vertexBuffer;
		if (pVertexBuffer->QueryInterface(GRLID_PPV_ARGS(&vertexBuffer)))
			return GRL_TRUE;
		D3D12_VERTEX_BUFFER_VIEW vertexBufferView = vertexBuffer->GetVertexBufferView(m_workingPipelineState->m_vertexStride);
		m_commandList->IASetVertexBuffers(0, 1, &vertexBufferView);
		m_commandList->IASetPrimitiveTopology(GRLTopologyToD3D12Topology(renderTopology));
		m_commandList->DrawInstanced(numVertices, 1, 0, 0);
		return GRL_FALSE;
	}
	GRL_RESULT GRLCD3D12Environment::DrawIndexedInstance(GRLIGraphicsBuffer* pVertexBuffer, GRLIGraphicsBuffer* pIndexBuffer, uint32_t numVertices, GRL_RENDER_TOPOLOGY renderTopology)
	{
		if (!m_duringDraw) return GRL_TRUE;
		GRLPtr<GRLCD3D12Buffer> vertexBuffer;
...
//		case GRL_GRAPHICS_TEXTURE_EXTENSION_MODE::CLAMP: D3D12_TEXTURE_ADDRESS_MODE_CLAMP;
	...
	GRL_RESULT GRLCD3D12Buffer::QueryInterface(GRLUUID uuid, void** ppObject)
	{
		...
		else if (uuid == GRLID_CD3D12Buffer)*ppObject = reinterpret_cast<GRLCD3D12Buffer*>(this);
		else return GRL_TRUE;
		AddRef();
		return GRL_FALSE;
	}
	D3D12_VERTEX_BUFFER_VIEW GRLCD3D12Buffer::GetVertexBufferView(uint32_t strideInBytes)
	{
	...
	GRL_RESULT GRLCD3D12DepthMap::QueryInterface(GRLUUID uuid, void** ppObject)
	{
		...
		else if (uuid == GRLID_CD3D12DepthMap) *ppObject = reinterpret_cast<GRLCD3D12DepthMap*>(this);
		else return GRL_TRUE;
		AddRef();
		return GRL_FALSE;
	}
